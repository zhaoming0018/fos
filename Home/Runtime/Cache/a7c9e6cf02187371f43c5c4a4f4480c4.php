			...
			<div class="article">
				...
				<div class="articleBody w9 center-block">
					<div class="content3_text">
						<ul class="list">
						<?php if(isset($_GET['category'])) { foreach($list as $key=>$value){ echo "
								<div>
									<h4>$key</h4>
								</div>
								"; for($i=0;$i<count($value);$i++){ echo "<li><a href='__APP__/Develop/show/title/".$value[$i]['denews_id']."'/>".$value[$i]['denews_title']."</a></li>"; } } }else{ ?>
						<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li><a href="__APP__/Develop/show/title/<?php echo ($vo["denews_id"]); ?>"><?php echo ($vo["denews_title"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
						<?php } ?>
						</ul>
					</div>
				</div>
