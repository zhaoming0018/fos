...
<form action=
<?php if($_GET["id"] == null): ?>'__APP__/Endept/doAdd'
<?php else: ?>'__APP__/Endept/doEdit/id/<?php echo ($_GET["id"]); ?>'<?php endif; ?> 
method='post'>
<div>
	<div class="text-center">添加组织机构</div>
	<hr />
	<div class="text-center">题目：<input type='text' name='name' <?php if($_GET["id"] != null): ?>value='<?php echo ($department['department_name_en']); ?>'<?php endif; ?>/></div>
	<hr />
	<div>
	<div class="text-center">内容</div>
		<script id="editor" type="text/plain" style="width:624px;height:500px;margin:0 auto;"><?php if($_GET["id"] != null): echo ($department['department_content_en']); endif; ?></script>
		</div>
