...
<form action=
<?php if($_GET["id"] == null): ?>'__APP__/Institution/doAdd'
<?php else: ?>'__APP__/Institution/doEdit/id/<?php echo ($_GET["id"]); ?>'<?php endif; ?> 
method='post'>
	<div class="text-center">添加组织机构</div>
	<hr />
	<div class="text-center">题目：<input type='text' name='name' <?php if($_GET["id"] != null): ?>value='<?php echo ($institution['institution_name']); ?>'<?php endif; ?>/></div>
	<hr />
	<div>
	<div class="text-center">内容</div>
		<script id="editor" type="text/plain" style="width:624px;height:500px;margin:0 auto;">
			<?php if($_GET["id"] != null): echo ($institution['institution_content']); endif; ?>
		</script>
	</div>
