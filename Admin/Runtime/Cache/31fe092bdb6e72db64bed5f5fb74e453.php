...
<form action='__URL__/doAdd' method='post' enctype='multipart/form-data'>
	<table class="table table-bordered">
		<tr><td>请选择文件:</td><td><input type='file'  name='photo' /></td></tr>
		<tr><td>请选择分类：</td><td><select name="file_category">
					<?php if(is_array($filecate)): $i = 0; $__LIST__ = $filecate;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value='<?php echo ($vo["filecate_id"]); ?>'><?php echo ($vo["filecate_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>	
				  </select></td>
		</tr>
		<!-- <input type='text' name='filename' /><br/> -->
		<tr><td colspan="2"><input type='submit' class="btn bcm tcw w3" value='提交' /></td></tr>
	</table>
</form>
				</block>
