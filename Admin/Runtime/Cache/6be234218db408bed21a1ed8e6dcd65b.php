...
<form action=
<?php if($_GET["id"] == null): ?>'__APP__/Develop/doAdd'
<?php else: ?>'__APP__/Develop/doEdit/id/<?php echo ($_GET["id"]); ?>'<?php endif; ?> 
method='post'>
<div>
	<div class="text-center">添加内容</div>
	<hr />
	<div class="text-center">题目：<input type='text' name='title' <?php if($_GET["id"] != null): ?>value='<?php echo ($denews['denews_title']); ?>'<?php endif; ?>/></div>
	<hr />
	<div class="text-center">分类：
		<select name="denews_category">
			<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value ="<?php echo ($vo["develop_id"]); ?>"  
				<?php if($vo["develop_id"] == $cate): ?>selected='selected'<?php endif; ?>
				><?php echo ($vo["develop_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
		</select>
	</div>
	<div>
	<div class="text-center">内容</div>
		<script id="editor" type="text/plain" style="width:624px;height:500px;margin:0 auto;"><?php if($_GET["id"] != null): echo ($denews['denews_content']); endif; ?></script>
		</div>
