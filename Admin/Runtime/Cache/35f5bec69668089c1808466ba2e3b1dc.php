...
<form action=
<?php if($_GET["id"] == null): ?>'__APP__/News/doAdd'
<?php else: ?>'__APP__/News/doEdit/id/<?php echo ($_GET["id"]); ?>'<?php endif; ?> 
method='post'>
	<div class="text-center" style="font-size:20px;">编辑新闻</div>
	<hr />
	<div class="text-center">题目：<input type='text' name='title' class="w5" <?php if($_GET["id"] != null): ?>value='<?php echo ($news['news_title']); ?>'<?php endif; ?>/></div>
	<hr />
	<div class="text-center">分类：<select name="category">
	...
	<div>
		...
		<div style="color:red;text-align:center;width:100%;">备注：上传图片宽度要小于等于400px,否则会发生文章排版错乱</div>
		<hr />
		<script id="editor" type="text/plain" style="width:624px;height:500px;margin:0 auto;"><?php if($_GET["id"] != null): echo ($news['news_content']); endif; ?></script>
	</div>
	<div class="w9 center-block mt20 mb20">
		<input type='submit' name='提交' class="btn bcm tcw w3" value="提交" />
...
<script type="text/javascript">
	$(document).ready(function(){
		$("[name='category']")[0].value="<?php echo ($news["news_category"]); ?>";
	});
</script>
